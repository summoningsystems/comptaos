...
import { coerceScalarToString } from "../ArithmeticHelper.mjs";
import { SimpleRangeValue } from "../../SimpleRangeValue.mjs";
import { isExtendedNumber } from "../InterpreterValue.mjs";
...
export class TextPlugin extends FunctionPlugin {
   ...
   */
  n(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('N'), arg => {
      var _a;
      const value = arg instanceof SimpleRangeValue ? (_a = arg.data[0]) === null || _a === void 0 ? void 0 : _a[0] : arg;
   ...
   */
  value(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('VALUE'), arg => {
      if (arg instanceof CellError) {
        return arg;
      }
   ...
   */
  textjoin(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('TEXTJOIN'), (delimiterArg, ignoreEmpty, ...textArgs) => {
      ...
      const result = parts.reduce((acc, part, i) => i === 0 ? part : acc + delimiters[(i - 1) % delimiters.length] + part, '');
  ...
  flattenArgToStrings(arg) {
    ...
    for (const val of values) {
      ...
      const coerced = coerceScalarToString(val);
      if (coerced instanceof CellError) {
        return coerced;
      }
      result.push(coerced);
