...
var _FormulaVertex = require("./DependencyGraph/FormulaVertex");
...
var _RenameSheetTransformer = require("./dependencyTransformers/RenameSheetTransformer");
...
class Operations {
  ...
  renameSheet(sheetId, newName) {
    ...
    if (mergedWithPlaceholderSheet !== undefined) {
      ...
      this.stats.measure(_statistics.StatType.TRANSFORM_ASTS, () => {
        const transformation = new _RenameSheetTransformer.RenameSheetTransformer(sheetId, mergedWithPlaceholderSheet);
  ...
  ensureItIsPossibleToMoveCells(sourceLeftCorner, width, height, destinationLeftCorner) {
    if ((0, _Cell.isColOrRowInvalid)(sourceLeftCorner) || !(isPositiveInteger(width) && isPositiveInteger(height) || isRowOrColumnRange(sourceLeftCorner, width, height)) || (0, _Cell.isColOrRowInvalid)(destinationLeftCorner) || !this.sheetMapping.hasSheetWithId(sourceLeftCorner.sheet) || !this.sheetMapping.hasSheetWithId(destinationLeftCorner.sheet)) {
  ...
  getClipboardCell(address) {
    ...
    } else if (vertex instanceof _DependencyGraph.ArrayFormulaVertex) {
    ...
    } else if (vertex instanceof _DependencyGraph.ScalarFormulaVertex) {
      return {
        type: _ClipboardOperations.ClipboardCellType.FORMULA,
        hash: this.parser.computeHashFromAst(vertex.getFormula(this.lazilyTransformingAstService))
      };
  ...
  adjustNamedExpressionEdges(namedExpression, expressionName, sheetId) {
    ...
    const localVertexId = maybeLocalVertexId !== null && maybeLocalVertexId !== void 0 ? maybeLocalVertexId : localVertex.idInGraph;
    const globalNamedExpression = this.namedExpressions.workbookNamedExpressionOrPlaceholder(expressionName);
    const {
      vertex: globalVertex,
      id: maybeGlobalVertexId
    } = this.dependencyGraph.fetchCellOrCreateEmpty(globalNamedExpression.address);
    const globalVertexId = maybeGlobalVertexId !== null && maybeGlobalVertexId !== void 0 ? maybeGlobalVertexId : globalVertex.idInGraph;
    for (const adjacentNode of this.dependencyGraph.graph.adjacentNodes(globalVertex)) {
      if (adjacentNode instanceof _DependencyGraph.ScalarFormulaVertex && adjacentNode.getAddress(this.lazilyTransformingAstService).sheet === sheetId) {
  ...
  updateNamedExpressionsForMovedCells(sourceLeftCorner, width, height, destinationLeftCorner) {
    ...
    for (const formulaAddress of targetRange.addresses(this.dependencyGraph)) {
      ...
      if (vertex instanceof _DependencyGraph.ScalarFormulaVertex && formulaAddress.sheet !== sourceLeftCorner.sheet) {
  ...
  allocateNamedExpressionAddressSpace() {
    this.dependencyGraph.addressMapping.addSheetWithStrategy(_NamedExpressions.NamedExpressions.SHEET_FOR_WORKBOOK_EXPRESSIONS, new _DependencyGraph.SparseStrategy(0, 0));
  }
  copyOrFetchGlobalNamedExpressionVertex(expressionName, sourceVertex, addedNamedExpressions) {
    ...
    if (expression === undefined) {
      ...
      if (sourceVertex instanceof _DependencyGraph.ScalarFormulaVertex) {
  ...
  isNotComputed(address) {
    ...
    return vertex instanceof _FormulaVertex.FormulaVertex && !vertex.isComputed();
  }
}
