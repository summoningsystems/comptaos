...
class BuildEngineFactory {
  ...
  static buildEngine(config, sheets = {}, inputNamedExpressions = [], stats = config.useStats ? new _statistics.Statistics() : new _statistics.EmptyStatistics()) {
    ...
    const lazilyTransformingAstService = new _LazilyTransformingAstService.LazilyTransformingAstService(stats, config.maxPendingLazyTransformations);
    ...
    const parser = new _parser.ParserWithCaching(config, functionRegistry, dependencyGraph.sheetReferenceRegistrar.ensureSheetRegistered.bind(dependencyGraph.sheetReferenceRegistrar));
    lazilyTransformingAstService.parser = parser;
    const unparser = new _parser.Unparser(config, sheetMapping, namedExpressions);
    ...
    const exporter = new _Exporter.Exporter(config, namedExpressions, sheetMapping, lazilyTransformingAstService);
