...
class FunctionPlugin {
  constructor(interpreter) {
    ...
    this.runFunction = (args, state, metadata, functionImplementation) => {
      ...
      const argumentValues = evaluatedArguments.map(([value]) => value);
  ...
  listOfScalarValues(asts, state) {
    ...
    for (const argAst of asts) {
      const isSyntacticallyEmpty = argAst.type === _parser.AstNodeType.EMPTY;
  ...
  calculateSingleCellOfResultArray(state, vectorizedArguments, argumentsMetadata, argumentIgnorableFlags, syntacticallyEmptyFlags, functionImplementation, returnNumberType) {
  ...
  coerceArgumentsToRequiredTypes(state, vectorizedArguments, argumentsMetadata, argumentIgnorableFlags, syntacticallyEmptyFlags) {
    ...
    for (let i = 0; i < argumentsMetadata.length; i++) {
      ...
      const argumentValue = rawArg === undefined ? argumentMetadata === null || argumentMetadata === void 0 ? void 0 : argumentMetadata.defaultValue : syntacticallyEmptyFlags[i] && (argumentMetadata === null || argumentMetadata === void 0 ? void 0 : argumentMetadata.emptyAsDefault) && (argumentMetadata === null || argumentMetadata === void 0 ? void 0 : argumentMetadata.defaultValue) !== undefined ? argumentMetadata.defaultValue : rawArg;
  ...
  evaluateArguments(args, state, metadata) {
    return metadata.expandRanges ? this.listOfScalarValues(args, state) : args.map(ast => [this.evaluateAst(ast, state), false, ast.type === _parser.AstNodeType.EMPTY]);
  }
