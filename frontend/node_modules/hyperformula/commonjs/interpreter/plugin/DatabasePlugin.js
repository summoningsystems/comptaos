 ...
 */
class DatabasePlugin extends _FunctionPlugin.FunctionPlugin {
  /**
   ...
   */
  withDatabaseArgs(ast, state, functionName, callback) {
    return this.runFunction(ast.args, state, this.metadata(functionName), (database, field, criteria) => {
      const fieldIndex = this.resolveFieldIndex(database, field);
      if (fieldIndex instanceof _Cell.CellError) {
        return fieldIndex;
      }
      const criteriaRows = this.buildDatabaseCriteria(database, criteria);
      if (criteriaRows instanceof _Cell.CellError) {
        return criteriaRows;
      }
    ...
    return this.withDatabaseArgs(ast, state, 'DCOUNT', (dbData, fieldIndex, criteriaRows) => {
      ...
      for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
        if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
          ...
          if (cellValue instanceof _Cell.CellError) {
            return cellValue;
          }
    ...
    return this.withDatabaseArgs(ast, state, 'DCOUNTA', (dbData, fieldIndex, criteriaRows) => {
      ...
      for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
        if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
          ...
          if (cellValue !== _InterpreterValue.EmptyValue && cellValue !== undefined && cellValue !== null) {
            count++;
          }
        }
      }
    ...
    return this.withDatabaseArgs(ast, state, 'DPRODUCT', (dbData, fieldIndex, criteriaRows) => {
      ...
      for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
        if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
          ...
          if ((0, _InterpreterValue.isExtendedNumber)(cellValue)) {
            product *= (0, _InterpreterValue.getRawValue)(cellValue);
            hasNumeric = true;
          }
        }
      }
    ...
    return this.withDatabaseArgs(ast, state, 'DSUM', (dbData, fieldIndex, criteriaRows) => {
      ...
      for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
        if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
          ...
          if ((0, _InterpreterValue.isExtendedNumber)(cellValue)) {
            sum += (0, _InterpreterValue.getRawValue)(cellValue);
          }
        }
      }
      return sum;
  ...
  dget(ast, state) {
    return this.withDatabaseArgs(ast, state, 'DGET', (dbData, fieldIndex, criteriaRows) => {
      ...
      if (matchedValue instanceof _Cell.CellError) {
        return matchedValue;
      }
      return matchedValue === _InterpreterValue.EmptyValue || matchedValue === undefined || matchedValue === null ? 0 : matchedValue;
    });
  }
  /**
    ...
    return this.withDatabaseArgs(ast, state, 'DMAX', (dbData, fieldIndex, criteriaRows) => {
      ...
      for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
        if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
          ...
          if ((0, _InterpreterValue.isExtendedNumber)(cellValue)) {
            const numValue = (0, _InterpreterValue.getRawValue)(cellValue);
            if (numValue > max) {
              max = numValue;
            }
            hasNumeric = true;
   ...
   */
  collectNumericValues(dbData, fieldIndex, criteriaRows) {
    const values = [];
    for (let rowIdx = 1; rowIdx < dbData.length; rowIdx++) {
      if (this.rowMatchesCriteria(dbData[rowIdx], criteriaRows)) {
        const cellValue = dbData[rowIdx][fieldIndex];
        ...
        if ((0, _InterpreterValue.isExtendedNumber)(cellValue)) {
          values.push((0, _InterpreterValue.getRawValue)(cellValue));
        }
      }
    }
    return values;
   ...
   */
  resolveFieldIndex(database, field) {
    if (field instanceof _Cell.CellError) {
      return field;
    }
    ...
    if ((0, _InterpreterValue.isExtendedNumber)(numericField)) {
      const index = Math.trunc((0, _InterpreterValue.getRawValue)(numericField));
   ...
   */
  buildDatabaseCriteria(database, criteria) {
    var _a;
    ...
    for (const criteriaHeader of criteriaHeaders) {
      if (criteriaHeader instanceof _Cell.CellError) {
        return criteriaHeader;
      }
    }
    // Map each criteria column to a database column index (or -1 if no match)
    const criteriaColumnMapping = criteriaHeaders.map(criteriaHeader => {
      if (typeof criteriaHeader !== 'string') {
        return -1;
      }
      const lowerHeader = criteriaHeader.toLowerCase();
      return dbHeaders.findIndex(dbHeader => typeof dbHeader === 'string' && dbHeader.toLowerCase() === lowerHeader);
    });
    const rows = [];
    for (let rowIdx = 1; rowIdx < criteriaData.length; rowIdx++) {
      ...
      for (let colIdx = 0; colIdx < criteriaHeaders.length; colIdx++) {
        ...
        const criterionValue = (_a = criteriaData[rowIdx]) === null || _a === void 0 ? void 0 : _a[colIdx];
        // Empty/blank criteria cell = match-all for that column — skip
        if (criterionValue === _InterpreterValue.EmptyValue || criterionValue === undefined || criterionValue === null) {
          continue;
        }
        // Propagate errors from the criteria cells instead of masking them as BadCriterion
        if (criterionValue instanceof _Cell.CellError) {
          return criterionValue;
        }
        const rawCriterionValue = (0, _InterpreterValue.isExtendedNumber)(criterionValue) ? (0, _InterpreterValue.getRawValue)(criterionValue) : criterionValue;
        const criterionPackage = this.interpreter.criterionBuilder.fromCellValue(rawCriterionValue, this.arithmeticHelper);
   ...
   */
  rowMatchesCriteria(dataRow, criteriaRows) {
    ...
    return criteriaRows.some(criteriaRow => {
      ...
      return criteriaRow.every(entry => {
        ...
        const rawValue = (0, _InterpreterValue.isExtendedNumber)(cellValue) ? (0, _InterpreterValue.getRawValue)(cellValue) : cellValue;
...
DatabasePlugin.implementedFunctions = {
  'DCOUNT': {
    method: 'dcount',
    parameters: databaseFunctionParameters
  },
  'DCOUNTA': {
    method: 'dcounta',
    parameters: databaseFunctionParameters
