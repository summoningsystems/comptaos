...
exports.PercentilePlugin = void 0;
 ...
 */
function percentileInclusive(sortedVals, k) {
 ...
 */
function percentileExclusive(sortedVals, k) {
  const n = sortedVals.length;
  const rank = k * (n + 1);
 ...
 */
class PercentilePlugin extends _FunctionPlugin.FunctionPlugin {
  /**
   ...
   */
  percentile(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('PERCENTILE.INC'), (range, k) => {
   ...
   * @returns interpolated percentile value, or CellError on invalid input
   */
  percentileExc(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('PERCENTILE.EXC'), (range, k) => {
      const vals = this.getSortedValues(range);
      if (vals instanceof _Cell.CellError) {
        return vals;
      }
   ...
   */
  quartile(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('QUARTILE.INC'), (range, quart) => {
   ...
   * @returns quartile value, or CellError on invalid input
   */
  quartileExc(ast, state) {
    return this.runFunction(ast.args, state, this.metadata('QUARTILE.EXC'), (range, quart) => {
      quart = Math.trunc(quart);
      if (quart < 1) {
        return new _Cell.CellError(_Cell.ErrorType.NUM, _errorMessage.ErrorMessage.ValueSmall);
      }
...
exports.PercentilePlugin = PercentilePlugin;
PercentilePlugin.implementedFunctions = {
  'PERCENTILE.INC': {
    method: 'percentile',
    parameters: [{
