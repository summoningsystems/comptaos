...
class FormulaParser extends _chevrotain.EmbeddedActionsParser {
  constructor(lexerConfig, resolveSheetReference) {
    ...
    this.columnRangeExpression = this.RULE('columnRangeExpression', () => {
      ...
      const firstAddress = this.ACTION(() => (0, _addressRepresentationConverters.columnAddressFromString)(startImage, this.formulaAddress, this.resolveSheetReference));
      const secondAddress = this.ACTION(() => (0, _addressRepresentationConverters.columnAddressFromString)(endImage, this.formulaAddress, this.resolveSheetReference));
    ...
    this.rowRangeExpression = this.RULE('rowRangeExpression', () => {
      ...
      const [startImage, endImage] = range.image.split(':');
      const firstAddress = this.ACTION(() => (0, _addressRepresentationConverters.rowAddressFromString)(startImage, this.formulaAddress, this.resolveSheetReference));
      const secondAddress = this.ACTION(() => (0, _addressRepresentationConverters.rowAddressFromString)(endImage, this.formulaAddress, this.resolveSheetReference));
      if (firstAddress === undefined || secondAddress === undefined) {
        return (0, _Ast.buildCellErrorAst)(new _Cell.CellError(_Cell.ErrorType.REF));
      }
    ...
    this.cellReference = this.RULE('cellReference', () => {
      ...
      const address = this.ACTION(() => {
        return (0, _addressRepresentationConverters.cellAddressFromString)(cell.image, this.formulaAddress, this.resolveSheetReference);
      });
    ...
    this.endRangeReference = this.RULE('endRangeReference', start => {
      ...
      const startAddress = this.ACTION(() => {
        return (0, _addressRepresentationConverters.cellAddressFromString)(start.image, this.formulaAddress, this.resolveSheetReference);
      });
      const endAddress = this.ACTION(() => {
        return (0, _addressRepresentationConverters.cellAddressFromString)(end.image, this.formulaAddress, this.resolveSheetReference);
      });
  ...
  handleOffsetHeuristic(args) {
    ...
    const topLeftCorner = new _CellAddress.CellAddress(cellArg.reference.col + colShift, cellArg.reference.row + rowShift, cellArg.reference.type, (_a = cellArg.reference.sheet) !== null && _a !== void 0 ? _a : undefined);
    ...
    if (width === 1 && height === 1) {
      ...
      const bottomRightCorner = new _CellAddress.CellAddress(topLeftCorner.col + width - 1, topLeftCorner.row + height - 1, topLeftCorner.type, topLeftCorner.sheet);
      const rangeSheetReferenceType = cellArg.reference.sheet == null ? _Ast.RangeSheetReferenceType.RELATIVE : _Ast.RangeSheetReferenceType.BOTH_ABSOLUTE;
      return (0, _Ast.buildCellRangeAst)(topLeftCorner, bottomRightCorner, rangeSheetReferenceType);
    }
  }
