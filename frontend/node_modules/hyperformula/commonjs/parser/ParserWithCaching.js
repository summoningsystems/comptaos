...
class ParserWithCaching {
  constructor(config, functionRegistry, resolveSheetReference) {
    this.config = config;
    ...
    this.formulaParser = new _FormulaParser.FormulaParser(this.lexerConfig, this.resolveSheetReference);
  ...
  computeHashFromTokens(tokens, baseAddress) {
    ...
    while (idx < tokens.length) {
      ...
      if ((0, _chevrotain.tokenMatcher)(token, _LexerConfig.CellReference)) {
        const cellAddress = (0, _addressRepresentationConverters.cellAddressFromString)(token.image, baseAddress, this.resolveSheetReference);
        ...
        const startAddress = (0, _addressRepresentationConverters.columnAddressFromString)(start, baseAddress, this.resolveSheetReference);
        const endAddress = (0, _addressRepresentationConverters.columnAddressFromString)(end, baseAddress, this.resolveSheetReference);
        ...
        const [start, end] = token.image.split(':');
        const startAddress = (0, _addressRepresentationConverters.rowAddressFromString)(start, baseAddress, this.resolveSheetReference);
        const endAddress = (0, _addressRepresentationConverters.rowAddressFromString)(end, baseAddress, this.resolveSheetReference);
        if (startAddress === undefined || endAddress === undefined) {
          hash = hash.concat('!REF');
        } else {
          hash = hash.concat(startAddress.hash(true), ':', endAddress.hash(true));
