...
class CrudOperations {
  ...
  addSheet(name) {
    ...
    const {
      sheetName,
      sheetId
    } = this.operations.addSheet(name);
    this.undoRedo.saveOperation(new _UndoRedo.AddSheetUndoEntry(sheetName, sheetId));
    return sheetName;
  }
  removeSheet(sheetId) {
    ...
    this.undoRedo.saveOperation(new _UndoRedo.RemoveSheetUndoEntry(originalName, sheetId, oldSheetContent, scopedNamedExpressions));
  }
  renameSheet(sheetId, newName) {
    ...
    } = this.operations.renameSheet(sheetId, newName);
    if (previousDisplayName !== undefined) {
      this.undoRedo.clearRedoStack();
      this.undoRedo.saveOperation(new _UndoRedo.RenameSheetUndoEntry(sheetId, previousDisplayName, newName, version, mergedPlaceholderSheetId));
    }
  ...
  ensureItIsPossibleToMoveRows(sheet, startRow, numberOfRows, targetRow) {
    ...
    if (!this.sheetMapping.hasSheetWithId(sheet) || (0, _Cell.isColOrRowInvalid)(sourceStart) || (0, _Cell.isColOrRowInvalid)(targetStart) || !isPositiveInteger(numberOfRows) || targetRow <= startRow + numberOfRows && targetRow >= startRow) {
  ...
  ensureItIsPossibleToMoveColumns(sheet, startColumn, numberOfColumns, targetColumn) {
    ...
    if (!this.sheetMapping.hasSheetWithId(sheet) || (0, _Cell.isColOrRowInvalid)(sourceStart) || (0, _Cell.isColOrRowInvalid)(targetStart) || !isPositiveInteger(numberOfColumns) || targetColumn <= startColumn + numberOfColumns && targetColumn >= startColumn) {
  ...
  ensureItIsPossibleToChangeContent(address) {
    if ((0, _Cell.isColOrRowInvalid)(address)) {
